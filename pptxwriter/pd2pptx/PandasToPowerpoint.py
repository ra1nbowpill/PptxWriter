...
class TableStyle:
    """Pre-set themes for PowerPoint Table
    """
...
def process_position_parameter(param):
    ...
    If an integer, returns the right instance of the Cm class to allow it
    to be treated as cm. If missing, then default to 4cm. Otherwise, pass
    through whatever it gets.
    """
    if param is None:
        return Cm(4)
...
def cell_set_align(cell, align):
    for p in cell.text_frame.paragraphs:
        p.alignment = align


def cell_set_size(cell, size):
    for p in cell.text_frame.paragraphs:
        p.font.size = size


def set_table_style(shp, style):
...
def df_to_table(slide, df, left=None, top=None, width=None, height=None,
                ...
                name=None, index=None, text_size=None, text_align=None,
                row_height=None, hide_zeros=None):
    """Converts a Pandas DataFrame to a PowerPoint table on the given
    Slide of a PowerPoint presentation.

    The table is a standard Powerpoint table, and can easily be modified
    with the Powerpoint tools, for example: resizing columns, changing
    formatting etc.
    ...
    Optional arguments:
     - left: Position of the left-side of the table, either as an integer
        in cm, or as an instance of a pptx.util Length class
        (pptx.util.Inches for example).
        Defaults to 4cm.
     ...
     - col_formatters: A n_columns element long list containing format
        ...
        For example ['', ',', '.2'] does no special formatting for the first
        column, uses commas as thousands separators in the second column, and
        formats the third column as a float with 2 decimal places.
     - rounding: A n_columns element long list containing a number for each
        integer column that requires rounding that is then multiplied by -1
        and passed to round(). The practical upshot of this is that you can
        give something like ['', 3, ''], which does nothing for the 1st and
        3rd columns (as they aren't integer values), but for the 2nd column,
        rounds away the 3 right-hand digits (eg. taking 25437 to 25000).
     - name: A name to be given to the table in the Powerpoint file.
        This is not displayed, but can help extract the table later to make
        further changes.
     """
    ...
    try:
        shp = slide.insert_table(rows + 1, cols)
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if text_align is not None:
                cell_set_align(cell, text_align)
            cell.text = text
            if text_size is not None:
                cell_set_size(cell, text_size)
...
def df_to_powerpoint(filename, df, **kwargs):
    ...
    Creates a new PowerPoint presentation with the given filename, with a
    single slide containing a single table with the Pandas DataFrame data
    in it.

    The table is a standard Powerpoint table, and can easily be modified with
    the Powerpoint tools, for example: resizing columns,
